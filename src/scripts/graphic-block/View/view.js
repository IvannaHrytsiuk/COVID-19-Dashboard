...
import { covidData, populationCountry } from '../../state';
...
export const ViewGraphicClass = class {
    init(label, data, color) {
        ...
        if (document.querySelector('.graphic').options[this.a].value.includes('100K') && sessionStorage.getItem('country') == null) {
            this.everyDayData = Object.values(data).map((_, i) => +(Object.values(data)[i] / (covidData.population / 100000)).toFixed(2));
            ...
            this.everyDayData = Object.values(data).map((_, i) => +(Object.values(data)[i] / (populationCountry / 100000)).toFixed(2));
        } else {
